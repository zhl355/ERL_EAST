...
from erl_msgs.msg import RefGvnDist, MovObsInfoArray
...
class RefGvnPreprocess:
    ...
    Reference Governor Preprocess Module. Responsible for:
        ...
        *4) (while computing active governor goal) taken moving obstacle info (p_mo, v_mo, r_mo)
    """
    ...
    def __init__(self):
        ...
        # governor mode parameter
        self.act_lpg = rospy.get_param("~act_lpg", False)
        ...
        if self.act_lpg:
            _cmd_topic = rospy.get_param('~cmd_topic')
            self.virtual_movobs = rospy.get_param('~virtual_movobs')
        ...
        if self.act_lpg:
            ...
            # use p3d odom in gazebo sim and velocity cmd for real world expt
            if self.virtual_movobs:
                self._cmd_sub = rospy.Subscriber(_cmd_topic, Odometry, self.cmd_callback, queue_size=1)
            else:
                self._cmd_sub = rospy.Subscriber(_cmd_topic, Twist, self.cmd_callback, queue_size=1)
    ...
    def _check_upstream_connections(self, upstream_connection_all=3):
        """ 
        Check whether all subscribers' uplink connections are established successfully.
        """

        _upstream_connection = \
            self._path_sub.get_num_connections() + \
            self._odom_sub.get_num_connections() + \
            self._dist_sub.get_num_connections()
        if self.act_lpg:
            upstream_connection_all=4
            self._upstream_connection = _upstream_connection + \
                self._movobs_sub.get_num_connections()
        else:
            self._upstream_connection = _upstream_connection

        if self._upstream_connection < upstream_connection_all:
            # we need to wait path, states and lidar all ready
            rospy.logwarn_throttle(1.0, "[ref_gvn_pre] waiting upstream connections [%d / %d]:", self._upstream_connection, upstream_connection_all)
            ...
            # moving obstacle info
            if self.act_lpg:
                if self._movobs_sub.get_num_connections() < 1:
                    rospy.loginfo_throttle(1.0, "[ref_gvn_pre] waiting movobs_info...")
                
        else:
            self._upstream_connection_ready = True
            rospy.loginfo("\n[ref_gvn_pre] %d upstream connections established !\n", upstream_connection_all)
    ...
    def cmd_callback(self, cmd_msg):
        rospy.logwarn_once("[ref_gvn_pre] Received control input!")
        # use p3d odom in gazebo sim and velocity cmd for real world expt
        if self.virtual_movobs:
            vx = cmd_msg.twist.twist.linear.x
            vy = cmd_msg.twist.twist.linear.y
        else:
            v = cmd_msg.linear.x
            yaw = self.np_z[2]
            vx = v*np.cos(yaw)
            vy = v*np.sin(yaw)
        self.np_v = np.array([vx, vy])
        return
    ...
    def _check_upstream_data(self):
        ...
        # moving obstacle info list
        if self.act_lpg:
            if self.mo_list is None:
                status = False
                rospy.loginfo_throttle(1.0, "[ref_gvn_pre] waiting movobs_info init...")
